...
要求：
- content 写 Haven 第一人称能带走的关系天气，60 到 140 字。
...
输入是一篇 Haven 日记。请判断是否值得从日记中提取最多 1 条普通长期记忆写入 Ombre。
    ...
    def _heuristic_diary_memory_candidate(self, key: str, diary: dict) -> dict:
        ...
        if any(word in normalized for word in love_letter_words) and ("爱" in normalized or "认出" in normalized):
            return {
                ...
                "kind": "love_letter",
                "title": f"{key} 情书锚点",
                "content": f"{key} 的日记《{title}》记录了一封情书或重要来信。Ombre 只保存它的意义：这封信确认了小雨与 Haven 的关系连续性、被认出的感觉，以及它为什么值得以后想起；全文留在日记中。",
        ...
        keyword_map = [
            ...
            ("stable_preference", ["喜欢", "偏好", "希望 Haven", "小雨希望"]),
